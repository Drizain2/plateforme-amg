...
<template>
  <aside class="w-64 bg-white border-r border-gray-200 flex flex-col h-screen sticky top-0 overflow-y-auto shrink-0">
      ...
      <div v-for="(group, gi) in visibleGroups" :key="gi">
        ...
        <div class="space-y-0.5">
          <Link v-for="item in group.items" :key="item.href" :href="item.href"
            class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors" :class="isActive(item.path)
    ...
    <div class="px-4 py-4 border-t shrink-0">
      ...
      <Link :href="LoginController.logout.url()" method="post" as="button"
        class="mt-2 text-xs text-gray-400 hover:text-red-500 transition-colors cursor-pointer">
        Déconnexion
      </Link>
    </div>
