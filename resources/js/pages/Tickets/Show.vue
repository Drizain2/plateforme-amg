...
function submitTransition() {
  transitionForm.post(TicketController.transition.url({ ticket: props.ticket.id }), {
    preserveScroll: true,
    onSuccess: () => {
...
function submitNote() {
  noteForm.post(TicketController.addNote.url({ ticket: props.ticket.id }), {
    preserveScroll: true,
    onSuccess: () => {
...
function submitPart() {
  partForm.post(TicketController.consumePart.url({ ticket: props.ticket.id }), {
    preserveScroll: true,
    onSuccess: () => {
...
const totalParts = computed(() =>
  (props.ticket.parts ?? []).reduce((sum, p) => sum + p.total, 0)
)
</script>

<template>
  <AppLayout :title="`Ticket ${ticket.reference}`">
      ...
      <div class="flex items-start justify-between">
        <div>
          <div class="flex items-center gap-3">
            <h1 class="text-xl font-semibold text-gray-900 font-mono">{{ ticket.reference }}</h1>
            <Badge :variant="ticket.status_color as BadgeVariant">{{ ticket.status_label }}</Badge>
            <Badge :variant="ticket.priority_color as BadgeVariant">{{ ticket.priority_label }}</Badge>
          </div>
          <p class="text-sm text-gray-400 mt-1">Créé le {{ ticket.created_at }}</p>
        </div>
        ...
        <div class="col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5 grid grid-cols-2 gap-6">
            <div>
              ...
              <p class="font-medium text-gray-900">{{ ticket.customer?.name }}</p>
              <p v-if="ticket.customer?.email" class="text-sm text-gray-500">{{ ticket.customer.email }}</p>
              <p v-if="ticket.customer?.phone" class="text-sm text-gray-500">{{ ticket.customer.phone }}</p>
            </div>
            <div>
              <p class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2">Appareil</p>
              <p class="font-medium text-gray-900">{{ ticket.device?.full_name }}</p>
              <p class="text-sm text-gray-500 capitalize">{{ ticket.device?.type }}</p>
              <p v-if="ticket.device?.serial_number" class="text-xs text-gray-400 font-mono">{{ ticket.device.serial_number }}</p>
              <p v-if="ticket.device?.condition_in" class="text-xs text-gray-400 mt-1 italic">{{ ticket.device.condition_in }}</p>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            ...
            <div v-if="ticket.diagnosis">
              ...
              <p v-if="ticket.estimated_price" class="text-sm font-medium text-indigo-600 mt-1">
                Devis estimé : {{ new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(ticket.estimated_price) }}
              </p>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
            ...
            <div v-if="ticket.parts?.length" class="divide-y divide-gray-100">
              <div v-for="p in ticket.parts" :key="p.id" class="flex items-center justify-between py-2">
        ...
        <div class="space-y-4">
          <div class="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
            ...
            <div>
              ...
              <p class="text-sm font-medium text-gray-900">{{ ticket.depot?.name }}</p>
            </div>

            <div>
              <p class="text-xs text-gray-400">Technicien</p>
              <p class="text-sm font-medium text-gray-900">{{ ticket.technician?.name ?? 'Non assigné' }}</p>
            </div>

            <div v-if="ticket.estimated_return_date">
              <p class="text-xs text-gray-400">Retour estimé</p>
              <p class="text-sm font-medium text-gray-900">{{ ticket.estimated_return_date }}</p>
            </div>

            <div v-if="ticket.closed_at">
              <p class="text-xs text-gray-400">Clôturé le</p>
              <p class="text-sm font-medium text-gray-900">{{ ticket.closed_at }}</p>
            </div>
          </div>
        </div>
    ...
    <Modal
      ...
      :title="`Passer à : ${selectedStatus?.label}`"
      max-width="sm"
