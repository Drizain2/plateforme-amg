...
const trialBannerStyle = computed(() => {
    ...
    if (d <= 1) return { bg: 'bg-red-50 border-b border-red-200', text: 'text-red-800', btn: 'bg-red-600 hover:bg-red-700 text-white' }
    if (d <= 3) return { bg: 'bg-orange-50 border-b border-orange-200', text: 'text-orange-800', btn: 'bg-orange-600 hover:bg-orange-700 text-white' }
    if (d <= 7) return { bg: 'bg-amber-50 border-b border-amber-200', text: 'text-amber-800', btn: 'bg-amber-600 hover:bg-amber-700 text-white' }
...
const trialLabel = computed(() => {
    const d = trialDaysLeft.value
    if (d === null) return ''
    if (d === 0) return "Votre essai gratuit expire aujourd'hui."
    if (d === 1) return 'Votre essai gratuit expire demain.'
...
watch(
  ...
  ([success, error, warning, info], [prevSuccess, prevError, prevWarning, prevInfo]) => {
    if (success && success !== prevSuccess) toast.success(success)
    if (error && error !== prevError) toast.error(error)
    if (warning && warning !== prevWarning) toast.warning(warning)
    if (info && info !== prevInfo) toast.info(info)
  },
)
</script>
